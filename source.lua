...
local siriusSettings = {
	...
	},
	{
		...
		categorySettings = {
			...
			{
				name = 'Limitar FPS quando desfocado',
				description = 'O TX definirá automaticamente seu FPS para 60 quando você sair da tela ou desfocar do Roblox.',
	...
	{
		name = 'Detections',
		description = 'GUIZIN TX bypass v1 detecta e previne qualquer coisa maliciosa ou possivelmente prejudicial a sua experiência.',
		color = Color3.new(0.705882, 0, 0),
		minimumLicense = 'Free',
		categorySettings = {
			{
				name = 'Spacial Tx v1',
				description = 'Suprima sons altos reproduzidos por qualquer fonte de áudio no jogo, em tempo real, com o Spatial Shield.',
				settingType = 'Boolean',
				minimumLicense = 'Pro',
				current = true,
			...
			{
				name = 'Moderador auto detect',
				description = 'Caso um administrador acessar a mesma sessão que a sua Você sera detectado imediatamente', 
				minimumLicense = 'Pro',
				current = true,
	...
	},
	{
		...
		categorySettings = {
			...
			{
				name = 'Log PlayerAdded e PlayerRemoving',
				description = 'Registre sempre que qualquer jogador sair ou entrar na sua sessão.',
				settingType = 'Boolean',
				current = false,
			...
			{
				name = 'Jogador adicionado e removendo URL do Webhook',
				description = 'Discord Webhook URL',
				settingType = 'Input',
				current = 'No Webhook',
...
local function addToQueue(file)
	...
	checkFolder()
	if not isfile(siriusValues.siriusFolder.."/Music/"..file) then queueNotification("Não foi possível localizar o arquivo", "Certifique-se de que seu arquivo de áudio esteja na pasta do nosso servidor /Música e que você esteja incluindo a extensão do arquivo (por exemplo, mp3 ou ogg).", 4370341699) return end
...
local function serverhop()
	...
	if #servers > 0 then
		...
		return queueNotification("Nenhum servidor encontrado", "Não conseguimos encontrar outro servidor, este pode ser o único servidor.", 4370317928)
	end
...
local function UpdateHome()
	...
	-- Home Title
	homeContainer.Title.Text = "BEM VINDO DE VOLTA, "..localPlayer.DisplayName
	...
	if not table.find(siriusValues.executors, string.lower(identifyexecutor())) then
		homeContainer.Interactions.Client.Subtitle.Text = "Este executor não foi verificado como suportado."
	end
...
local function start()
	...
	if script_key and not (Essential or Pro) then
		queueNotification("Erro de licença", "Detectamos uma chave sendo colocada acima da string de carga do BISTECONE HUB, no entanto, sua chave parece ser inválida. Faça uma solicitação de suporte em t.me/Guizintx para resolver isso em minutos.", "do")
	end
...
players.PlayerAdded:Connect(function(player)
	...
	if checkSetting("MODERADOR DETECTADO ⚠️").current and Pro then
		...
		if siriusValues.currentCreator == "group" then
			for _, role in pairs(siriusValues.administratorRoles) do 
				if string.find(string.lower(roleFound), role) then
					...
					queueNotification("ATENÇÃO UM ADMINISTRADOR ENTROU NA MESMA SESSÃO QUE VOCÊ ESTA POR FAVOR SAIR DO MAPA PARA EVITAR BANIMENTOS!", siriusValues.currentGroup .." "..roleFound.." ".. player.DisplayName .." juntou-se à sua sessão", 3944670656) -- change to group name
				end
			end
		end
	end
